...
abstract class OldMem extends InstanceFactory {
	...
	OldMem(String name, StringGetter desc, int extraPorts) {
		super(name, desc);
		currentInstanceFiles = new WeakHashMap<Instance,File>();
		setInstancePoker(OldMemPoker.class);
	...
	abstract OldMemState getState(InstanceState state);
	abstract OldMemState getState(Instance instance, CircuitState state);
	...
	public void paintInstance(InstancePainter painter) {
		...
		if (painter.getShowState()) {
			OldMemState state = getState(painter);
	...
	public void loadImage(InstanceState instanceState, File imageFile)
			throws IOException { 
		OldMemState s = this.getState(instanceState);
	...
	protected Object getInstanceFeature(Instance instance, Object key) {
		if (key == MenuExtender.class) return new OldMemMenu(this, instance);
		return super.getInstanceFeature(instance, key);
	}
	
	static class OldMemListener implements HexModelListener {
		Instance instance;
		
		OldMemListener(Instance instance) { this.instance = instance; }
