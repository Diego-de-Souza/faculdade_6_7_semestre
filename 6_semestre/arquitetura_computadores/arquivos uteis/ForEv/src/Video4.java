...
// 320 x 240 pixel LCD display with 8bpp color (byte addressed)
class Video4 extends ManagedComponent {
    ...
    public static final Attribute BLINK_OPTION = Attributes.forOption("cursor",
		    Strings.getter("Cursor"), BLINK_OPTIONS);
    public static final Attribute RESET_OPTION = Attributes.forOption("reset",
		    Strings.getter("Reset Behavior"), RESET_OPTIONS);

    private static final Attribute[] ATTRIBUTES = { BLINK_OPTION, RESET_OPTION };

    private static class Factory extends AbstractComponentFactory {
        private Factory() { }
        public String getName() { return "LCD Video 320x240x24"; }
        public String getDisplayName() { return "LCD Video 320x240x24"; }
        public AttributeSet createAttributeSet() {
            return AttributeSets.fixedSet(ATTRIBUTES, new Object[] { BLINK_OPTIONS[0], RESET_OPTIONS[0] });
        }
        public Component createComponent(Location loc, AttributeSet attrs) { return new Video4(loc, attrs); }
        public Bounds getOffsetBounds(AttributeSet attrs) { return Bounds.create(-334, -124, 334, 254); }
	...
	static final BitWidth NINE = BitWidth.create(9); 
    ...
    private Video4(Location loc, AttributeSet attrs) {
        super(loc, attrs, 6);
        ...
        setEnd(P_X, getLocation().translate(-140, 130), NINE, EndData.INPUT_ONLY);
	...
	void drawVideo(ComponentDrawContext context, int x, int y, State state, Object blink_option, Object reset_option) {
		...
		g.drawRoundRect(x, y, 334-1, 254-1, 6, 6);
		for (int i = P_CLK+1; i <= P_RST; i++)
			context.drawPin(this, i);
		g.drawRect(x+6, y+6, 322-1, 242-1);
    ...
    private State getState(CircuitState circuitState) {
        ...
        if (state == null) {
            state = new State(new BufferedImage(320, 240, BufferedImage.TYPE_INT_RGB));
