...
class OldMemState implements InstanceData, Cloneable, HexModelListener {
	...
	OldMemState(OldMemContents contents) {
	...
	@Override
	public OldMemState clone() {
		try {
			OldMemState ret = (OldMemState) super.clone();
	...
	private void setBits(int addrBits, int dataBits) {
		if (contents == null) {
			contents = OldMemContents.create(addrBits, dataBits);
		} else {
	...
	public OldMemContents getContents() {
		return contents;
	}
