...
public class OldRom extends OldMem {
	public static Attribute<OldMemContents> CONTENTS_ATTR = new ContentsAttribute();
	
	// The following is so that instance's OldMemListeners aren't freed by the
	// garbage collector until the instance itself is ready to be freed.
	private WeakHashMap<Instance,OldMemListener> memListeners;
	
	public OldRom() {
		super("OldROM", Strings.getter("romComponent"), 0);
		setIconName("rom.gif");
		memListeners = new WeakHashMap<Instance,OldMemListener>();
	}
	...
	public AttributeSet createAttributeSet() {
		return new OldRomAttributes();
	}

	@Override
	OldMemState getState(Instance instance, CircuitState state) {
		OldMemState ret = (OldMemState) instance.getData(state);
		if (ret == null) {
			OldMemContents contents = getMemContents(instance);
			ret = new OldMemState(contents);
	...
	@Override
	OldMemState getState(InstanceState state) {
		OldMemState ret = (OldMemState) state.getData();
		if (ret == null) {
			OldMemContents contents = getMemContents(state.getInstance());
			ret = new OldMemState(contents);
			state.setData(ret);
		}
		return ret;
	...
	HexFrame getHexFrame(Project proj, Instance instance, CircuitState state) {
		return OldRomAttributes.getHexFrame(getMemContents(instance), proj);
	}
	
	// TODO - maybe delete this method?
	OldMemContents getMemContents(Instance instance) {
		return instance.getAttributeValue(CONTENTS_ATTR);
	}

	@Override
	public void propagate(InstanceState state) {
		OldMemState myState = getState(state);
	...
	protected void configureNewInstance(Instance instance) {
		...
		OldMemContents contents = getMemContents(instance);
		OldMemListener listener = new OldMemListener(instance);
		memListeners.put(instance, listener);
		contents.addHexModelListener(listener);
	}
	
	private static class ContentsAttribute extends Attribute<OldMemContents> {
		public ContentsAttribute() {
			super("contents", Strings.getter("romContentsAttr"));
		}

		@Override
		public java.awt.Component getCellEditor(Window source, OldMemContents value) {
			if (source instanceof Frame) {
				Project proj = ((Frame) source).getProject();
				OldRomAttributes.register(value, proj);
			}
		...
		@Override
		public String toDisplayString(OldMemContents value) {
			return Strings.get("romContentsValue");
		}

		@Override
		public String toStandardString(OldMemContents state) {
		...
		@Override
		public OldMemContents parse(String value) {
			...
			try {
				...
				OldMemContents ret = OldMemContents.create(addr, data);
	...
	private static class ContentsCell extends JLabel
		...
		Window source;
		OldMemContents contents;
		
		ContentsCell(Window source, OldMemContents contents) {
		...
		public void mouseClicked(MouseEvent e) {
			...
			HexFrame frame = OldRomAttributes.getHexFrame(contents, proj);
