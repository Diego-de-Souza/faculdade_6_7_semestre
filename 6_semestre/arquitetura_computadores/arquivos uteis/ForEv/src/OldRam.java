...
public class OldRam extends OldMem {
	...
	private static Attribute<?>[] ATTRIBUTES = {
		OldMem.ADDR_ATTR, OldMem.DATA_ATTR, ATTR_BUS
	};
	...
	public OldRam() {
		super("OldRAM", Strings.getter("ramComponent"), 3);
		setIconName("ram.gif");
	...
	OldMemState getState(InstanceState state) {
		...
		OldRamState myState = (OldRamState) state.getData();
		if (myState == null) {
			OldMemContents contents = OldMemContents.create(addrBits.getWidth(), dataBits.getWidth());
			Instance instance = state.getInstance();
			myState = new OldRamState(instance, contents, new OldMemListener(instance));
	...
	OldMemState getState(Instance instance, CircuitState state) {
		...
		OldRamState myState = (OldRamState) instance.getData(state);
		if (myState == null) {
			OldMemContents contents = OldMemContents.create(addrBits.getWidth(), dataBits.getWidth());
			myState = new OldRamState(instance, contents, new OldMemListener(instance));
	...
	HexFrame getHexFrame(Project proj, Instance instance, CircuitState circState) {
		OldRamState state = (OldRamState) getState(instance, circState);
		return state.getHexFrame(proj);
	}

	@Override
	public void propagate(InstanceState state) {
		OldRamState myState = (OldRamState) getState(state);
		BitWidth dataBits = state.getAttributeValue(DATA_ATTR);
	...
	private static class OldRamState extends OldMemState
			implements InstanceData, AttributeListener {
		private Instance parent;
		private OldMemListener listener;
		private HexFrame hexFrame = null;
		private OldClockState clockState;

		OldRamState(Instance parent, OldMemContents contents, OldMemListener listener) {
			super(contents);
			this.parent = parent;
			this.listener = listener;
			this.clockState = new OldClockState();
		...
		@Override
		public OldRamState clone() {
			OldRamState ret = (OldRamState) super.clone();
			ret.parent = null;
		...
		public void attributeValueChanged(AttributeEvent e) {
			...
			BitWidth addrBits = attrs.getValue(OldMem.ADDR_ATTR);
			BitWidth dataBits = attrs.getValue(OldMem.DATA_ATTR);
	...
	public static class Logger extends InstanceLogger {
		...
		public Value getLogValue(InstanceState state, Object option) {
			if (option instanceof Integer) {
				OldMemState s = (OldMemState) state.getData();
