...
public class OldMemPoker extends InstancePoker {
	private OldMemPoker sub;

	@Override
	public boolean init(InstanceState state, MouseEvent event) {
		Bounds bds = state.getInstance().getBounds();
		OldMemState data = (OldMemState) state.getData();
	...
	private static class DataPoker extends OldMemPoker {
		int initValue;
		int curValue;

		private DataPoker(InstanceState state, OldMemState data, long addr) {
			data.setCursor(addr);
			initValue = data.getContents().get(data.getCursor());
			curValue = initValue;
			
			Object attrs = state.getInstance().getAttributeSet();
			if (attrs instanceof OldRomAttributes) {
				Project proj = state.getProject();
				if (proj != null) {
					((OldRomAttributes) attrs).setProject(proj);
				}
			}
		}
		...
		public Bounds getBounds(InstancePainter painter) {
			OldMemState data = (OldMemState) painter.getData();
		...
		private void moveTo(OldMemState data, long addr) {
	...
	private static class AddrPoker extends OldMemPoker {
		@Override
		public Bounds getBounds(InstancePainter painter) {
			OldMemState data = (OldMemState) painter.getData();
