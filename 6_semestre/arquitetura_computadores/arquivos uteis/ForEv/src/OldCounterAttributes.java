...
class OldCounterAttributes extends AbstractAttributeSet {
	private AttributeSet base;
	
	public OldCounterAttributes() {
		base = AttributeSets.fixedSet(new Attribute<?>[] {
				StdAttr.WIDTH, OldCounter.ATTR_MAX, OldCounter.ATTR_ON_GOAL,
				StdAttr.EDGE_TRIGGER,
				StdAttr.LABEL, StdAttr.LABEL_FONT
			}, new Object[] {
				BitWidth.create(8), Integer.valueOf(0xFF),
				OldCounter.ON_GOAL_WRAP,
				StdAttr.TRIG_RISING,
	...
	public void copyInto(AbstractAttributeSet dest) {
		((OldCounterAttributes) dest).base = (AttributeSet) this.base.clone();
	}
	...
	public <V> void setValue(Attribute<V> attr, V value) {
		...
		if (attr == StdAttr.WIDTH) {
			...
			Integer oldValObj = base.getValue(OldCounter.ATTR_MAX);
			...
			if (newW > oldW) {
				...
				if (v != oldVal) {
					...
					base.setValue(OldCounter.ATTR_MAX, newValObj);
					fireAttributeValueChanged(OldCounter.ATTR_MAX, newValObj,null);
				}
			}
			fireAttributeValueChanged(StdAttr.WIDTH, newWidth,null);
		} else if (attr == OldCounter.ATTR_MAX) {
			int oldVal = base.getValue(OldCounter.ATTR_MAX).intValue();
		...
		if (newMax != null) {
			base.setValue(OldCounter.ATTR_MAX, newMax);
			fireAttributeValueChanged(OldCounter.ATTR_MAX, newMax,null);
		}
	}
