...
public class OldShiftRegister extends InstanceFactory {
	...
	public OldShiftRegister() {
		super("Old Shift Register", Strings.getter("shiftRegisterComponent"));
		...
		setInstanceLogger(OldShiftRegisterLogger.class);
		setInstancePoker(OldShiftRegisterPoker.class);
	}
	...
	private OldShiftRegisterData getData(InstanceState state) {
		...
		if (data == null) {
			data = new OldShiftRegisterData(width, length);
			state.setData(data);
		} else {
	...
	public void propagate(InstanceState state) {
		...
		OldShiftRegisterData data = getData(state);
	...
	public void paintInstance(InstancePainter painter) {
		...
		if (parallel) {
			...
			if (painter.getShowState()) {
				if (wid <= 4) {
					OldShiftRegisterData data = getData(painter);
				...
				String b = Strings.get("%sx%s" + len + wid);
