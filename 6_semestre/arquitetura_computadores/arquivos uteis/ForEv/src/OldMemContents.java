...
class OldMemContents implements Cloneable, HexModel {
	private static final int PAGE_SIZE_BITS = 12;
	private static final int PAGE_SIZE = 1 << PAGE_SIZE_BITS;
	private static final int PAGE_MASK = PAGE_SIZE - 1;
	
	static OldMemContents create(int addrBits, int width) {
		return new OldMemContents(addrBits, width);
	}
	...
	private int mask;
	private OldMemContentsSub.ContentsInterface[] pages;
	
	private OldMemContents(int addrBits, int width) {
		listeners = null;
	...
	//
	@Override
	public OldMemContents clone() {
		try {
			OldMemContents ret = (OldMemContents) super.clone();
			ret.listeners = null;
			ret.pages = new OldMemContentsSub.ContentsInterface[this.pages.length];
	...
	public boolean isClear() {
		for (int i = 0; i < pages.length; i++) {
			OldMemContentsSub.ContentsInterface page = pages[i];
			if (page != null) {
	...
	public void set(long addr, int value) {
		...
		if (old != val) {
			if (pages[page] == null) {
				pages[page] = OldMemContentsSub.createContents(PAGE_SIZE, width);
			}
	...
	public void set(long start, int[] values) {
		...
		if (pageStart == pageEnd) {
			...
			OldMemContentsSub.ContentsInterface page = pages[pageStart];
			...
			for (int i = pageStart + 1; i < pageEnd; i++, offs += PAGE_SIZE) {
				...
				if (page == null) {
					...
					if (!allZeroes) {
						page = OldMemContentsSub.createContents(PAGE_SIZE, width);
						pages[i] = page;
					}
				}
				if (page != null) {
			...
			if (endOffs >= 0) {
				...
				OldMemContentsSub.ContentsInterface page = pages[pageEnd];
				if (!page.matches(vals, startOffs, mask)) {
	...
	private void clearPage(int index) {
		OldMemContentsSub.ContentsInterface page = pages[index];
	...
	public void setDimensions(int addrBits, int width) {
		...
		OldMemContentsSub.ContentsInterface[] oldPages = pages;
		int pageCount;
		int pageLength;
		...
		pages = new OldMemContentsSub.ContentsInterface[pageCount];
		if (oldPages != null) {
			int n = Math.min(oldPages.length, pages.length);
			for (int i = 0; i < n; i++) {
				if (oldPages[i] != null) {
					pages[i] = OldMemContentsSub.createContents(pageLength, width);
		...
		if (pageCount == 0 && pages[0] == null) {
			pages[0] = OldMemContentsSub.createContents(pageLength, width);
		}
		fireMetainfoChanged();
	}
	...
	private void ensurePage(int index) {
		if (pages[index] == null) {
			pages[index] = OldMemContentsSub.createContents(PAGE_SIZE, width);
		}
	}
}
