...
public class OldCounter extends InstanceFactory {
	...
	public OldCounter() {
		super("OldCounter", Strings.getter("counterComponent"));
		setOffsetBounds(Bounds.create(-30, -20, 30, 40));
		setIconName("counter.gif");
		setInstancePoker(OldRegisterPoker.class);
		setInstanceLogger(OldRegisterLogger.class);
	...
	public AttributeSet createAttributeSet() {
		return new OldCounterAttributes();
	}
	...
	public void propagate(InstanceState state) {
		OldRegisterData data = (OldRegisterData) state.getData();
		if (data == null) {
			data = new OldRegisterData();
			state.setData(data);
		}
		...
		if (state.getPortValue(CLR) == Value.TRUE) {
			...
			boolean ct = state.getPortValue(CT) != Value.FALSE;
	...
	public void paintInstance(InstancePainter painter) {
		...
		OldRegisterData state = (OldRegisterData) painter.getData();
		...
		if (painter.getShowState()) {
			...
			b = Strings.get("(%sb)" + widthVal.getWidth());
		}
